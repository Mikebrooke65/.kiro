...
| **Two pre-existing bugs found building Gant (2026-09-03)** | 🔴 Found, not fixed | (1) `teams-api.ts` line 168: a genuine TS type error (missing `is_coach` on `TeamMemberWithTeam`), invisible to `npm run build` since Vite doesn't type-check — confirmed via `git diff` unrelated to the Gant build. (2) `ProtectedRoute` only checks the synchronous global `user.role`, so a Manager with per-team `is_coach` authority (who correctly sees the Coaching tab in nav) is redirected away if they navigate directly to `/coaching` or `/ai-coach` — pre-existing for `/coaching`, not introduced by Gant. **Pick up before V1 go-live** — see `.kiro/specs/gant-ai-feedback-assistant/tasks.md` Task 5's write-up for full detail |
